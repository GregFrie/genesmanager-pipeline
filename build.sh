...
echo "==> Install Google Chrome (with keyring)"
install -d -m 0755 /etc/apt/keyrings
curl -fsSL https://dl.google.com/linux/linux_signing_key.pub \
  | tee /etc/apt/keyrings/google-linux-signing-key.pub >/dev/null
...
curl -fsSL "https://chromedriver.storage.googleapis.com/${CHROMEDRIVER_VERSION}/chromedriver_linux64.zip" \
  -o /tmp/chromedriver.zip
unzip -o /tmp/chromedriver.zip -d /usr/local/bin
chmod +x /usr/local/bin/chromedriver
rm -f /tmp/chromedriver.zip

# środowisko (część hostów korzysta z tych zmiennych)
